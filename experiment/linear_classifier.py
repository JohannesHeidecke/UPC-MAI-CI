    ...
    for folder in folders:
        ...
        for image in os.listdir(folder):
            ...
            try:
                ...
                pass
                # print('Could not
                 # read image: ', image_file," : ", e, '- Skipping..')
...
def runClassifier(train_folder, test_folder):

    train_folders, test_folders = get_folder_names(train_folder, test_folder)

    #warning: max_images should be > sum(no.of all images in all folders)
    train_dataset, train_labels = load(train_folders, 100, 30500) 
    ...
    acc = float(result['TP'] + result['TN']) / (result['TP'] + result['TN']+ result['FP'] + result['FN'])
