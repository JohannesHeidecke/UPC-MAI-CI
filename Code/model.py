...
class DCGAN(object):
    def __init__(self, sess, image_size=108, is_crop=True,
                 ...
                 checkpoint_dir=None, sample_dir=None, d=1.0, k=100):
        """
    ...
    def load_mnist(self):
        ...
        d = float(self.d)
        k = self.k
        print('CONFIG: generating samples for digit '+str(d))
        print('CONFIG: generating based on ' + str(k) + ' real samples')
        in_trY = np.where(trY == d)
        in_teY = np.where(teY == d)
        trX = trX[in_trY]
        trY = trY[in_trY]
        teX = teX[in_teY]
        teY = teY[in_teY]
        ...
        for i in range(0,k):
            ...
            sample[0] = trX[i]
            save_images(sample, [1, 1], self.sample_dir+'/original_%s.png' % (i))
